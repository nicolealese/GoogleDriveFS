...
function trashFile() {
  var fileName = prompt("Enter the file name : ", "file name here");
  if (fileName === null) {
    console.log("no valid file name");
  }
  else {
  trashOldFile(fileName);
  }
}
 ...
 *
 * @param {String} fileName Name of the file to trash.
 */
function trashOldFile(fileName) {
  var request = gapi.client.drive.files.list({
    "q": "title = '" + fileName + "'"
  });
  request.execute(function(resp) {
...
function writeAFile(fileName, callback) {
  var request = gapi.client.drive.files.list({
    "q": "title = '" + fileName + "'"
  });
  request.execute(function(resp) {
